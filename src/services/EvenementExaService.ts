...
export class EvenementExaService {
  ...
  public verifEvenement(evenements: any, numdoss) {
    ...
    db.openDatabase({
      ...
      db.executeSql("select * from EvenementExa where numdoss like '" + numdoss + "'", [])
        .then(result => {
  ...
  private _insertEvenements(evenements: Array<Evenement>): void {
   ...
   public deleteEvenementExas(numdoss) {
    ...
    db.openDatabase({
      ...
      db.executeSql("delete from EvenementExa where  numdoss like '" + numdoss + "'", [])
        .then(() => {
