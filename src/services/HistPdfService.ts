...
export class HistPdfService {
  ...
  public verifHistPdf(numDoss, codeClinique): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      ...
      db.openDatabase({
        ...
        db.executeSql("select count(*) as sum from HistPdf where numDoss like '" + numDoss + "' and codeClinique like '" + codeClinique + "'and codeClinique like '" + codeClinique + "'", [])
          .then(result => {
  ...
  public getHistPdfs(histDossiers: any, numDoss, codeClinique): Promise<HistDossier> {
    return new Promise<HistDossier>(resolve => {
      ...
      db.openDatabase({
        ...
        db.executeSql("select * from HistPdf where numDoss like '" + numDoss + "' and codeClinique like '" + codeClinique + "'and codeClinique like '" + codeClinique + "'", [])
          .then(result => {
  ...
  private _insertHistSigneCourbes(histDossiers: Array<HistDossier>): void {
    ...
    db.openDatabase({
      ...
      for (let key in histDossiers) {
        ...
        db.executeSql('insert into HistPdf (numDoss ,date ,codeClinique) values (?,?,?)', [
  ...
  public deleteHistPdfs(numDoss, codeClinique) {
    let db = new SQLite();
    db.openDatabase({
      name: 'clinisys.db',
      location: 'default' // the location field is required
    }).then(() => {
      db.executeSql("delete from HistPdf where numDoss like '" + numDoss + "' and codeClinique like '" + codeClinique + "'and codeClinique like '" + codeClinique + "'", [])
        .then(() => {
