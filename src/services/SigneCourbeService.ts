...
export class ClientService {
  ...
  public verifSigneCourbe(signeCourbes: any, numdoss,codeClinique) : Promise<boolean> {
    return new Promise<boolean>(resolve => {
      ...
      db.openDatabase({
        ...
        db.executeSql("select count(*) as sum from SigneCourbe where numdoss like '" + numdoss + "'and codeClinique like '" + codeClinique + "'", [])
          .then(result => {
          ...
          .catch(error => {
            ...
            alert('Error 0 SigneCourbe  ' + error);
            resolve(false);
            return false;
          })
      });
  ...
  public getSigneCourbes(signeCourbes: any, numdoss,codeClinique) {
    ...
    db.openDatabase({
      ...
      db.executeSql("select * from SigneCourbe where numdoss like '" + numdoss + "'and codeClinique like '" + codeClinique + "'", [])
        .then(result => {
        ...
        .catch(error => {
          console.error('Error opening database', error);
          alert('Error 1 signeCourbe  ' + error);
        })
    });
    db.close();
  ...
  private _insertSigneCourbes(signeCourbes: Array<SigneCourbe>,numdoss,codeClinique): void {
    ...
    db.openDatabase({
      ...
      for (let key in signeCourbes) {
        ...
        db.executeSql('insert into SigneCourbe (code,codePosologie,designation' +
      ...
      console.error('Error opening database', error);
      alert('Error 2 signeCourbe ' + error);
    });
    db.close();
  }
  ...
  public deleteSigneCourbes(numdoss,codeClinique) {
    ...
    db.openDatabase({
      ...
      db.executeSql("delete from SigneCourbe where numdoss like '" + numdoss + "'and codeClinique like '" + codeClinique + "'", [])
        .then(() => {
          alert("Suppression de table signeCourbe est terminé avec succes");
        })
        .catch(error => {
          console.error('Error opening database', error);
          alert('Error 3 signeCourbe  ' + error);
        })
    });
    db.close();
    return this.signeCourbe;
